...
export const useBucketsStore = defineStore('buckets', {
  ...
  actions: {
    async ensureLoaded(): Promise<void> {
      // The stream carries every bucket's current row, so subscribing is what keeps this
      // store current afterwards -- there is no timer anywhere that asks again.
    ...
    async getBucketWithEvents({
      ...
      // A view asking for one bucket's events is a view showing that bucket, so it
      // follows the bucket's own stream from here on and does not ask again.
      useStreamStore().follow(id);
