  ...
  methods: {
    ...
    toggleAutoRefresh: function () {
      if (!this.autorefresh || this.running_interval) {
        ...
        this.running_interval();
        this.autorefresh = false;
        this.running_interval = null;
      } else {
        console.log('Starting autorefresh');
        this.autorefresh = true;
        // Driven by the change stream rather than by a timer: the store names the buckets
        // that moved, and this view rechecks when one of the two it reads is among them.
        this.running_interval = this.$watch(
          () => useStreamStore().dirty['aw-watcher-window_' + this.hostname],
          () => this.check()
        );
      }
    },
